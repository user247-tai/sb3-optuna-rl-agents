...
from utils.env_utils import make_env, make_normalized_vec_env
...
VEC_NORMALIZE_PATH = os.path.join(MODEL_DIR, f"ars_{ENV_ID}_vecnormalize.pkl")
...
def objective(trial: optuna.Trial):
    ...
    eval_env = None
    try:
        train_env = make_normalized_vec_env([make_env(ENV_ID, i, "log/ars") for i in range(NUM_ENV)])
        eval_env = make_normalized_vec_env([lambda: Monitor(gym.make(ENV_ID))], training=False)
...
def train_agent(log_dir = "log/ars"):
    ...
    train_env = make_normalized_vec_env([make_env(ENV_ID, i, log_dir) for i in range(NUM_ENV)])
    eval_env = make_normalized_vec_env([lambda: Monitor(gym.make(ENV_ID))], training=False)
    ...
    model = sb3_contrib.ARS(
        ...
        seed=42,
        n_eval_episodes=5,
        n_delta=best_params["n_delta"],
        n_top=best_params["n_top"],
        learning_rate=best_params["learning_rate"],
        delta_std=best_params["delta_std"],
        zero_policy=best_params["zero_policy"],
    )
...
def test_agent():
    env = DummyVecEnv([lambda: Monitor(gym.make(ENV_ID, render_mode="human"))])
